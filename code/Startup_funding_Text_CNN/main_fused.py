...
from util_fused import MyDatasetLoader, Metric
from model_fused import TextCNN
import torch
...
class Template:
    ...
    def train(self, epoch, dataset, mode='train'):
        ...
        for index, data in enumerate(dataiter):
            ...
            input_basic = data['input_basic'].to(self.device)
            input_labels = data['input_labels'].to(self.device)
            predict_labels = self.model(input_ids,input_basic)
            if mode == 'train':
        ...
        if mode != 'train':
            ...
            description = "{} score: {}, loss: {:.4f}, accuracy: {:4f}, auc: {:4f}, best auc socre: {:.4f}".format(mode.title(), epoch, loss, acc, auc, self.metircs.best_valid if mode == 'valid' else self.metircs.best_test)
            print(description)
    ...
    def evaluate(self, epoch, dataset, mode='test'):
        ...
        description1 = "{} score: {}, loss: {:.4f}, accuracy: {:4f}, best auc socre: {:.4f}".format(mode.title(), epoch,loss, acc, self.metircs.best_test)
        print(description1)
        description2 = "{} score: {}, loss: {:.4f}, accuracy: {:4f}, auc: {:4f}, best auc socre: {:.4f}".format(mode.title(), epoch,loss, acc, auc, self.metircs.best_test)
        print(description2)
    ...
    def forward(self, train_data, valid_data, test_data):
        ...
        return acc, auc,self.metircs.best_test
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='SU1')
