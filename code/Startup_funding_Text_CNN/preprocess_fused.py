...
class Template:
    ...
    def read_split_file(self, mode):
        ...
        for line in a:
            ...
            seq = ' '.join(cols[52:])
            # print(seq)
            label, basic_feature, text = int(line[0]), [float(num) for num in cols[1:52]], self.clean_str(seq).split()
            res.append((text, basic_feature,label))
        return res
    ...
    def execute(self, data_list):
        ...
        for key, data in data_list.items():
            cur_res = []
            for line, basic_feature,label in data:
                res_line = []
                for word in line:
                    word = self.normalize_word(word)
                    res_line.append(self.alphabet.get_index(word))
                cur_res.append((res_line, basic_feature, label))
            # self.alphabet.close()
    ...
    def run_read_file(self):
        ...
        if self.datatype == 'train_test':
            ...
            # print(self.read_split_file)
            # print(data_list)
            # if data_list[1] is None:
            #     X, X_basic, y = zip(*data_list[0])
